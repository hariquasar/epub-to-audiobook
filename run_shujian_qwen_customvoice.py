...
def open_project(source: Path, project: Path) -> tuple[dict, list[str]]:
    ...
    if m.get("style_sha256") != hashlib.sha256(STYLE.encode()).hexdigest():
        raise SystemExit(
            "Narration style differs from the manifest; choose a new output folder to avoid mixing voices."
        )
...
def main() -> None:
    ...
    for entry, source_chunk in zip(m["chunks"], chunks):
        ...
        for attempt in range(1, 4):
            ...
            try:
                ...
                save_manifest(project, m)
                print(
                    f"PASS {entry['index']}/{total} attempt={attempt} duration={entry['audit']['duration_seconds']:.2f}s",
                    flush=True,
                )
                break
    ...
    with sf.SoundFile(project / m["chunks"][0]["audio"], "r") as first:
        with sf.SoundFile(
            combined, "w", samplerate=first.samplerate, channels=first.channels, format="FLAC", subtype="PCM_16"
        ) as out:
    ...
    print(
        f"COMPLETE {combined} chunks={audit['passed_chunks']} duration={audit['generated_duration_seconds']:.2f}s",
        flush=True,
    )
